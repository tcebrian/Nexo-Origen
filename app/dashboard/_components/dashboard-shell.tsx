...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = (isRestaurantUser ? restaurantUserMenuItems : menuItems).filter((item) =>
    ...
    <DashboardControlsProvider>
      <main className="relative h-screen overflow-hidden bg-[#05030A] font-sans text-white antialiased">
        ...
        <div className="relative z-[1] flex h-screen">
          <aside className="flex w-[250px] shrink-0 flex-col border-r border-white/[0.08] bg-black/35 backdrop-blur-2xl">
          ...
          <section className={`nexo-radial-depth flex-1 overflow-y-auto px-8 py-6 ${isRestaurantUser ? "bg-[#05030A]" : ""}`}>
